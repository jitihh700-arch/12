(function() {
    ...
    function addFootballRecordsCategory() {
        ...
        description.textContent = 'Top 10 des records & statistiques';
        const count = document.createElement('span');
        count.className = 'questions-count';
        count.textContent = '10 records';
        card.append(title, description, count);
        grid.append(card);

        const stats = document.querySelector('.v4-home-stats');
        if (stats) [...stats.children].forEach(item => { if (/26 catégories/i.test(item.textContent)) item.textContent = '27 catégories'; });
        document.querySelectorAll('.v4-feature-list li').forEach(item => { if (/26 catégories existantes/i.test(item.textContent)) item.textContent = '27 catégories disponibles.'; });
    }
    ...
    function enrichCategories() {
        document.querySelectorAll('.category-card[data-category]').forEach(card => {
            ...
            image.height = 400;
            image.loading = 'lazy';
            image.onerror = () => { image.hidden = true; card.classList.add('v4-category-missing-image'); };
    ...
    function suggestionLabel(card) { return card.querySelector('h3')?.textContent?.trim() || card.dataset.category || ''; }
    ...
    function renderCategorySuggestions(query, activeFilter) {
        ...
        const matches = categoryCards().filter(card => {
        ...
        matches.forEach(card => {
            ...
            button.type = 'button';
            button.textContent = suggestionLabel(card);
            button.addEventListener('click', event => {
                event.preventDefault();
                event.stopPropagation();
                input.value = suggestionLabel(card);
    ...
    function filterCategories() {
        ...
        categoryCards().forEach(card => {
            const visible = (!query || card.dataset.v4Search.includes(query)) && (activeFilter === 'all' || card.dataset.v4Theme === activeFilter);
            card.hidden = !visible;
    ...
    function bindExplorer() {
        ...
        search?.addEventListener('keydown', event => { if (event.key === 'Enter') { event.preventDefault(); filterCategories(); } });
        clear?.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); search.value = ''; search.focus({ preventScroll: true }); filterCategories(); });
        searchForm?.addEventListener('submit', event => { event.preventDefault(); filterCategories(); });
        document.querySelectorAll('[data-v4-filter]').forEach(button => button.addEventListener('click', event => {
            event.preventDefault(); event.stopPropagation();
            document.querySelectorAll('[data-v4-filter]').forEach(item => item.classList.remove('is-active'));
            button.classList.add('is-active'); filterCategories();
        }));
        document.querySelectorAll('.category-card[data-category]').forEach(card => card.addEventListener('click', event => {
            event.stopPropagation(); event.preventDefault();
            const category = card.getAttribute('data-category');
            if (category && typeof window.showGamePanel === 'function') window.showGamePanel(category);
        }));
        document.querySelectorAll('.category-card[data-category]').forEach(card => card.addEventListener('keydown', event => {
            if (event.key !== 'Enter' && event.key !== ' ') return;
            event.preventDefault();
            const category = card.getAttribute('data-category');
            if (category && typeof window.showGamePanel === 'function') window.showGamePanel(category); else card.click();
        }));
    }
    ...
    function setCurrentRoute(route) {
        ...
        document.querySelectorAll('[data-v4-view]').forEach(view => view.classList.toggle('is-v4-current', view.getAttribute('data-v4-view') === viewRoute));
    ...
    function bindNavigation() {
        document.addEventListener('click', event => {
            ...
            if (emptyAnchor) { event.preventDefault(); return; }
            const menuButton = event.target.closest('#v4-menu-toggle');
            if (menuButton) { event.preventDefault(); setMobileMenu(menuButton.getAttribute('aria-expanded') !== 'true'); return; }
            const link = event.target.closest('[data-v4-route]');
            if (!link) return;
            const route = link.getAttribute('data-v4-route');
            if (!route) return;
            event.preventDefault(); setMobileMenu(false); goToRoute(route, { focus: true });
        });
